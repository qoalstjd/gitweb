$(function(){
    ...
    function brandPrevSlider(){               
        var bgUrl2 = $('.brandList>li').attr('data-bg2')
            $('#brand').css('background-image','url('+bgUrl2+')')
        $('.brandList>li:last').prependTo('.brandList').css('margin-left','-1080px').animate({marginLeft:0},500)
        $('.brandList>li').eq(2).animate({opacity:1},500).siblings().animate({opacity:0.3},500)
    }
    
    brandNextSlider()
   
    $('#brand .nextBtn').click(function(){   
        brandNextSlider()
    })

    $('#brand .prevBtn').click(function(){        
        brandPrevSlider()
    })

    $('.controls>li>a').click(function(e){e.preventDefault()})

    $('.controls>li').each(function(index){
        var date = $(this).find('a').attr('data-date')
        $(this).click(function(){
            $(this).find('a').text(date).parent().addClass('select').siblings().removeClass('select').find('a').text('.')
            $('.newsWrap>li').eq(index).fadeIn(500).siblings().css('display','none')
        })
